...
if(metapp_shop_ym==1){
	...
	$('#metapp_shop_addtocart').live('click',function(){
		...
		for(var i=1;i<=slistcount;i++){
          var spar=$('input[name=spara'+i+']').val(); //xiaochao 07-03
		  {shopdescription+=spar+' ';checked=1;}
		  if(spar==""){
		   	alert(alertxt+'!!!');
			return false;
		   }
	...
	$(document).ready(function(){
		...
		if($('#Orders_submitted').size()>0){
			$('#Orders_submitted').submit(function() {
				...
				var vals = $('#Orders_submitted').formSerialize();
				$.post(urls, vals ,function(data) {
					window.location.href=urls+'&ordernumber='+data;
				});
				return false;
			});
			var formlistval = $('#Orders_submitted').formSerialize()

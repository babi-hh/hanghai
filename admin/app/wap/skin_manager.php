...
$rurls='../app/wap/skin_manager.php?anyid='.$anyid.'&cs='.$cs.'&lang='.$lang.'&labetype'.$labetype;
if($action=='openthis'){
	$oldfile      =$depth."../../templates/$wap_skin_user/lang/language_$met_index_type.ini";   
	$newfile      =$depth."../../templates/$wap_skin_user/lang/language_$lang.ini"; 
	if(!file_exists($newfile)){  
		if($met_index_type==$lang){
			$query="select * from $met_lang where lang!='metinfo' order by id asc";
			$langs=$db->get_all($query);
			$oldfile=$depth."../../templates/$wap_skin_user/lang/language_$langs[0][mark].ini";  
		}
	...
	if($skin_name=='')metsave('-1',$lang_js8,$depth);
	if($skin_file=='')metsave('-1',$lang_js9,$depth);
	$skin_m=$db->get_one("SELECT * FROM $met_skin_table WHERE skin_file='$skin_file'");
	if($skin_m)metsave('-1',$lang_temexists1,$depth);
	...
	for($i=0;$i<$k;$i++){
		...
		if($skin_name[$allidlist[$i]]=='')metsave('-1',$lang_js8,$depth);
		if($skin_file[$allidlist[$i]]=='')metsave('-1',$lang_js9,$depth);
	...
	if($action_type=="del"){
		...
		if(!$skin_m)metsave('-1',$lang_dataerror,'',$depth);
		if($skin_m[skin_file]==$met_skin_user)metsave('-1',$lang_temexists2,$depth);
    ...
    $query = "SELECT * FROM $met_skin_table where devices='1' order BY id LIMIT $from_record, $list_num";
    $result = $db->query($query);
	while($list = $db->fetch_array($result)){
		$cssfile=$depth."../../templates/".$list[skin_file]."/images/css/css.inc.php";
	...
	$page_list = $rowset->link("skin_manager.php?anyid=".$anyid."&lang=".$lang."&cs=$cs&page=");
	...
	include template('app/wap/skin');footer();
}

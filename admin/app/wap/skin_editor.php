...
$file_name=$depth."../../templates/{$skin_file}/lang/language_{$lang}.ini";
...
if($action=='modify'){
	...
	metsave('../app/wap/skin_editor.php?anyid='.$anyid.'&lang='.$lang.'&cs='.$cs.'&skin_file='.$skin_file.'&id='.$id,'',$depth);
}
				...
				switch($hngy5){
					case 1:
						foreach($met_class1 as $key=>$val2){
							...
							if($val2[module]<6&&!$val2[if_in]&&($val2[wap_ok]||!$met_wap_ok)){
					...
					case 2:
						foreach($met_class1 as $key=>$val2){
							if(!$val2[wap_ok]&&$met_wapshowtype)continue;
							if($val2[module]<7&&!$val2[if_in]&&($val2[wap_ok]||!$met_wap_ok)){
							$select=$val['value']==$val2[id].'-cm'?'selected':'';
							$val['inputhtm'].="<option value='".$val2[id]."-cm' {$select} class='c1'>==".$val2[name]."==</option>";
							foreach($met_class2[$val2['id']] as $key=>$val3){
								if($val3[module]<7&&!$val3[if_in]&&($val3[wap_ok]||!$met_wap_ok)){
					...
					case 3:
						foreach($met_class1 as $key=>$val2){
							if(!$val2[wap_ok]&&$met_wapshowtype)continue;
							if(($val2[module]==2||$val2[module]==3||$val2[module]==5)&&!$val2[if_in]&&($val2[wap_ok]||!$met_wap_ok)){
							$select=$val['value']==$val2[id].'-cm'?'selected':'';
							$val['inputhtm'].="<option value='".$val2[id]."-cm' {$select} class='c1'>==".$val2[name]."==</option>";
							foreach($met_class2[$val2['id']] as $key=>$val3){
								if(($val3[module]==2||$val3[module]==3||$val3[module]==5)&&!$val3[if_in]&&($val3[wap_ok]||!$met_wap_ok)){
								$select2=$val['value']==$val3[id].'-cm'?'selected':'';
								$val['inputhtm'].="<option value='".$val3[id]."-cm' {$select2} class='c2'>".$val3[name]."</option>";
								foreach($met_class3[$val3['id']] as $key=>$val4){
									if($val4[wap_ok]||!$met_wap_ok){
					...
					case 4:
						foreach($met_class1 as $key=>$val2){
							if(!$val2[wap_ok]&&$met_wapshowtype)continue;
							if($val2[wap_ok]||!$met_wap_ok){
							$select=$val['value']==$val2[id].'-cm'?'selected':'';
							$val['inputhtm'].="<option value='".$val2[id]."-cm' {$select} class='c1'>==".$val2[name]."==</option>";
							foreach($met_class2[$val2['id']] as $key=>$val3){
								if($val3[wap_ok]||!$met_wap_ok){
								$select2=$val['value']==$val3[id].'-cm'?'selected':'';
								$val['inputhtm'].="<option value='".$val3[id]."-cm' {$select2} class='c2'>".$val3[name]."</option>";
								foreach($met_class3[$val3['id']] as $key=>$val4){
									if($val4[wap_ok]||!$met_wap_ok){
...
include template('app/wap/skin_editor');
footer();

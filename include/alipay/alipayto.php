...
function error($lang){
echo("<script type='text/javascript'>alert('$lang');window.close(); </script>");
die();
}
if(!preg_match("/(^[1-9][0-9]*$)|(^0.[0-9]*$)|(^[1-9][0-9]*.[0-9]*$)/",$_GET['total'])){error('充值金额错误');}
...
if($_GET['type']=='recharge'){
	$out_trade_no = date('YmdHis').$_GET['orders'];
}
else{
	$out_trade_no = $_GET['orders'];
}
//订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
if($_GET['type']=='recharge'){
$subject      = "充值-".$_GET['name'];
}
else{
$subject      = "订单-".$_GET['orders'];
}
//订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
if($_GET['type']=='recharge'){
$body      = "充值-".$_GET['name'].'-'.$_GET['lang'];
}
else{
$body      = "订单-".$_GET['orders'];
}

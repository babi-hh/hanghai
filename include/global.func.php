...
global $met_sqlinsert,$id,$class1,$class2,$class3;
	...
	if(is_array($string)){
		...
		$string = str_ireplace("set", "\se\t", $string);
		$string = str_ireplace("where", "\wh\ere", $string);
						...
						if($index=='index'){
							...
							if($lang==$met_index_type){
								...
								if($id){
									...
									$query="select * from $met_column where foldername='".daddslashes($dir_dirname)."' and lang='$lang' and (classtype='1' or releclass!='0') order by id asc";
								}

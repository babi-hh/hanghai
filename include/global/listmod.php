		...
		foreach($dbparaname as $key=>$val2){
			...
			if($prices){
					if(!strstr($prices, "-")){
						...
						$results=$result[0];
						$serch_sql .= " and exists(select * from $met_plist where module=3 and $met_plist.paraid='$val2[id]' and $met_plist.listid=$dbname.id and $met_plist.info > ".intval($prices_sql[0])." and $met_plist.info < $results) "; //bychao
						$serchpage .= "&".$prices1."=".trim($$prices1);
					}
